...
##### Data name
If you want mrap to extract the data name from the ``code_string``, please indicate it as 'data = my_name', although many R packages also allow for merely 'my_name'. If the data name is not deteced, it can be added manually to the instance (see Modify the instance). 
...
```
In the case below we cannot extract the name, and you can add the target label manually to the instance: 

```{r}
"package::function(one_vector, another_vector)"
```
You will get a warning reminding to do it, which can be safely ignored. 
